...
test('parseCAP parses basic file', () => {
  const doc = unwrap(parseCAP(simpleCAP))
...
test('parseCAP collects errors', () => {
  const cap = `$VideoStandard PAL

invalid:00:05:00
Hello`

  const result = parseCAP(cap, { onError: 'collect' })
...
test('parseCAP reports missing tab separator', () => {
  const cap = `$VideoStandard PAL
